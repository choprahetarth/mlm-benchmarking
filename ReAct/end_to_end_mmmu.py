...
for i in idxs:
    ...
    print("Actual Answer is =", info['gt_answer'])
    ...
    for col, value in enumerate(row_data):
        if col == 11:  # Assuming column index 9 is the image column
            ...
            if isinstance(value, list):
                # Decode each image in the list and save them in next columns
                for i, img_data in enumerate(value):
                    image_stream = BytesIO(base64.b64decode(img_data))
                    img = Image(image_stream)
                    img.width = 250
                    img.height = 250
                    ws.add_image(img, ws.cell(row=row + 2, column=col + 1 + i).coordinate)
            else:
            ...
            if not isinstance(value, list):  # Check if value is not a list
